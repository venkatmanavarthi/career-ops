...
import { getMissingSetupFiles } from './lib/setup-store.mjs';
...
const missingSetupFiles = getMissingSetupFiles(projectRoot);

// 1. Check cv.md exists
const cvPath = join(projectRoot, 'cv.md');
if (missingSetupFiles.includes('cv.md')) {
...
if (missingSetupFiles.includes('config/profile.yml')) {
...
if (missingSetupFiles.includes('modes/_profile.md')) {
  errors.push('modes/_profile.md not found. Generate it through the form-based setup so personalization stays in the user layer.');
}

if (missingSetupFiles.includes('portals.yml')) {
  errors.push('portals.yml not found. Generate it through the form-based setup or copy templates/portals.example.yml.');
}

if (missingSetupFiles.includes('data/applications.md')) {
  warnings.push('data/applications.md not found yet. The form-based setup creates it automatically.');
}
