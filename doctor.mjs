...
function checkCv() {
  if (!getMissingSetupFiles(projectRoot).includes('cv.md')) {
...
function checkProfile() {
  if (!getMissingSetupFiles(projectRoot).includes('config/profile.yml')) {
...
function checkProfileOverrides() {
  if (!getMissingSetupFiles(projectRoot).includes('modes/_profile.md')) {
  ...
  return {
    ...
    fix: [
      'Run setup in the web UI or copy modes/_profile.template.md to modes/_profile.md',
      'Then customize it through the form-based settings screen',
    ],
  };
}

function checkPortals() {
  if (!getMissingSetupFiles(projectRoot).includes('portals.yml')) {
...
function checkTracker() {
  if (!getMissingSetupFiles(projectRoot).includes('data/applications.md')) {
  ...
  return {
    ...
    fix: [
      'Run setup in the web UI to create the tracker automatically',
      'Or create data/applications.md with the default tracker table',
    ],
  };
}
